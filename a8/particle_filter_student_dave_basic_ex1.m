...
T = 150; % duration the chase (i.e. number of iterations).
...
fig_h = figure(1)
clf
ax_h = subplot(1,2,1,'Parent',fig_h)
plot(1,x_P,'.k','markersize',5)
xlabel('time step')
ylabel('flight position')
ax_h = subplot(1,2,2,'Parent',fig_h)
hist(x_P,100)
...
for t = 1:T
    ...
    %{
    %the after
    ax_h = subplot(1,3,3,'Parent',fig_h)
    plot(0,x_P_update,'.k','markersize',5)
    hold on;
...
RMSE = sqrt(mean((x_est_out - x_out).^2));
% title_str = sprintf('Particle Filter Example with:\nx_R=%.2f, N=%.0f for T=%.0f\nRMSE=%0.4f', x_R, N, T, RMSE);
% title(title_str)
